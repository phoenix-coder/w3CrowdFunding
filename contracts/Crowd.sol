...
contract CrowdFunding{
    ...
    mapping (uint => Request) public requests;
    event showOndashboard (uint id, string title, string description, uint deadline, uint target, uint raisedAmount, bool approved, address receiver);
    mapping (address => uint) public showToken;
    ...
    function generateToken (string memory name, string memory symbol, uint256 decimal) public {
        require(msg.sender == owner,"Only owner");
        crowdToken = new Token(name, symbol, decimal);
        showToken[msg.sender] = decimal;
    }

    function approveToken(uint amount) public returns (uint){
        require(msg.sender == owner, "Only owner can approve tokens.");
        crowdToken.approve(address(this), amount);
        return crowdToken.allowance(msg.sender, address(this));
    }
    ...
    function donate(uint id) public payable {
        ...
        require(crowdToken.allowance(owner, address(this)) >= tokenAmount, "CrowdFunding contract not approved to transfer tokens.");

   
        crowdToken.transferFrom(owner, msg.sender, tokenAmount);

        showToken[owner] -= 2;
        showToken[msg.sender] += 2;
    }
